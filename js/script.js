...
function insertData(elem, obj) {
    ...
    if (obj.departments !== undefined) {
        ...
        if (isEmpty(deps)) {
            throw new depsEmptyError("К сожалению в вашем регионе нет отделов");
        }
...
function showMessage(obj, error) {
    ...
    if (error !== null) {
        ...
        if (error instanceof depsEmptyError) {
            elemError.querySelector("p").innerHTML = error.message;
        }
...
class depsEmptyError extends Error {
    constructor(message) {
      super(message);
      this.name = "depsEmptyError";
    }
}
